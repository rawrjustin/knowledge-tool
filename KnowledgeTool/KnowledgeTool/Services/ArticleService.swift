...
actor ArticleService {
    ...
    private func extractContent(from html: String) -> String {
        ...
        for pattern in contentPatterns {
            if let range = cleanedHTML.range(of: pattern, options: [.regularExpression, .caseInsensitive]),
               let match = try? NSRegularExpression(pattern: pattern, options: .dotMatchesLineSeparators).firstMatch(
                in: cleanedHTML,
        ...
        if content.isEmpty {
            if let bodyRange = cleanedHTML.range(of: #"<body[^>]*>(.*?)</body>"#, options: [.regularExpression, .caseInsensitive, .dotMatchesLineSeparators]) {
